...
Ошибки перевода, заметные в интерфейсе, `fetch-l10n.py` правит поверх
скачанных файлов (таблица `CORRECTIONS`) при каждом запуске. Так в русском
Firefox заголовок панели открытых вкладок звучал «из открытых вкладок».
...
Скрипт открывает несколько сайтов (значки на новой вкладке берутся из
истории), задаёт избранное, снимает главный экран в тёмной и светлой схеме,
панель оформления и покадрово — анимацию слияния вкладок в группы. Снимки
ложатся в `docs/screenshots/readme/`, английские — с суффиксом `-en`.

Кадр рисует сам браузер, без рабочего стола и курсора. У такого снимка есть
особенность: элементы страницы вкладки с `opacity` меньше единицы в него не
попадают, хотя на экране видны. Скрипт на время снимка заменяет им
`opacity` на `filter: opacity()` — выглядит так же.

Всплывающие панели — отдельные окна, и в такой снимок они не попадают.
Панель группы вкладок скрипт снимает с экрана (`tools/screenshot.ps1`) и
обрезает по окну браузера.

Marionette не видит вкладок Vantara (Firefox он узнаёт по ID приложения),
поэтому код для страницы вкладки скрипт выполняет скриптом вкладки
(`loadFrameScript`).
